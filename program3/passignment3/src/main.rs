...
fn main(){
    ...
    //accept n -- also check n errors
    let mut _x: i32 = 0;
    loop {
        //These 3 lines accepts user n as String
        println!("Enter N (the number of process nodes to generate each time): ");
        let mut n = String::new();
        io::stdin().read_line(&mut n).expect("failed to read line");

        //Here we convert String to int and also do error handling for strings
        let _num: i32 = match n.trim().parse() {
            Ok(_num) => {
                if _num < 0{
                    println!("Invalid n :( .Needs to be greater than 0.");
                    continue;
                }
                _x = _num;
                break;
    ...
    //accept s (sleep time in ms between generations) -- also check errors
    let mut _y: i32 = 0;
    loop {
        println!("Enter S (the sleep time in ms between generations): ");
        let mut s = String::new();
        io::stdin().read_line(&mut s).expect("failed to read line");

        //Here we convert String to int and also do error handling for strings
        let _num: i32 = match s.trim().parse() {
            Ok(_num) => {
                if _num < 0{
                    println!("Invalid n :( .Needs to be greater than 0.");
                    continue;
    ...
    let mut _z: i32 = 0;
    loop {
        println!("Enter M (number of times the producer should generate N nodes): ");
        let mut s = String::new();
        io::stdin().read_line(&mut s).expect("failed to read line");
